...
app.post("/api/data", async (req, res) => {
  ...
  if (typeof device_id !== "string") {
    return res.status(400).json({
      error: "device_id must be a string like 'device_1'"
    });
  }
  ...
  try {
    ...
    await db.query(
      `INSERT INTO sensor_data (device_id, soil, temperature, humidity, rssi)
       VALUES (?, ?, ?, ?, ?)`,
      [device_id, soil, temperature, humidity, rssi]
    );
    ...
    if (!jsonData.devices[device_id]) {
      jsonData.devices[device_id] = { readings: [] };
    }
    ...
    // Keep last 100 readings in JSON
    jsonData.devices[device_id].readings.unshift(newReading);
    if (jsonData.devices[device_id].readings.length > 100) {
      jsonData.devices[device_id].readings = jsonData.devices[device_id].readings.slice(0, 100);
    }
